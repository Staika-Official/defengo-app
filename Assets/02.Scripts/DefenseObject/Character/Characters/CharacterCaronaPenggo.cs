...
{
    public class CharacterCaronaPenggo : Character
    {
        public ObscuredFloat additiveDamageValue = 0.35f;
        ...
        public override IEnumerator ActionSequence()
        {
            ...
            {
                IceProjectile projectile = GameManager.Instance.objectPoolManager.GetObject<IceProjectile>(projectileName);
                projectile.transform.localPosition = transform.localPosition;

                float distance = Calculator.DistanceCheck(currentGridPos - Calculator.TransformationVector(targetCache[i].prevPositionIdx));

                float damage = totalDamage + (distance * (starGradeIndex + 1)) * additiveDamageValue;
        ...
        public override void SetCharacterInfo()
        {
            ...
            float calculatedRange = characterData.detectRange + (characterData.characterClassLevel - 1) * characterData.classUpFactor[1];
            attackRange = Mathf.Pow(calculatedRange, 2);
        ...
        public override void UpgradeLevel(int upgradeIndex)
        {
            ...
            SetAttackDamage();
            additiveDamageValue += (upgradeIndex - 1) * characterData.powerFactor[1];
        }
