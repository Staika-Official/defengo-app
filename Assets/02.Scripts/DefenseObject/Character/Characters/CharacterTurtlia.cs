...
{
    public class CharacterTurtlia : Character
    {
        ...
        public override IEnumerator ActionSequence()
        {
            ...
            for (int i = 0; i < targetCache.Count; i++)
            {
                if (isThrow)
                {
                    WaterSlowProjectile projectile = GameManager.Instance.objectPoolManager.GetObject<WaterSlowProjectile>(projectileName);
                    ...
                    projectile.Initialize(this, targetCache[i], isCritical, projectileSpeed, slowDownValue, slowDownDuration, particleName);
                }
            }
        ...
        public override void SetCharacterInfo()
        {
            ...
            slowDownValue = characterData.characterUniqueValue[0];
            slowDownDuration = characterData.characterUniqueValue[1];
            
            isAttackType = true;
            targetCount = (int)characterData.targetCount;
            basicAttackDamage = characterData.attackDamage;
            attackSpeed = characterData.attackSpeed;
            criticalRange = characterData.criticalRange;
